...
namespace OwnaudioLegacy.Sources
{
    ...
    public partial class SourceSound : ISource
    {
        ...
        /// <summary>
        /// Maximum number of frames to keep in the queue to prevent memory overflow.
        /// ~230ms buffer at 512 samples per buffer.
        /// </summary>
        private const int MAX_QUEUE_SIZE = 10;

        /// <summary>
        /// Counter for dropped frames due to queue overflow.
        /// </summary>
        ...
        public void SubmitSamples(float[] samples)
        {
            ...
            {
                // CRITICAL: Check queue size before adding more frames to prevent OOM
                if (SourceSampleData.Count >= MAX_QUEUE_SIZE)
                {
                    // Drop oldest frame (FIFO overflow handling)
                    if (SourceSampleData.TryDequeue(out var dropped))
                    {
                        SimpleAudioBufferPool.Return(dropped);
                        _droppedFrames++;

                        // Log warning every 10 dropped frames
                        if (_droppedFrames % 10 == 0)
                        {
                            Debug.WriteLine($"WARNING: SourceSound queue overflow - {_droppedFrames} frames dropped");
                        }
                    }
                }
                ...
                catch (Exception ex)
                {
                    ...
                    SimpleAudioBufferPool.Return(buffer); // Return buffer on error
                }
            }
        }
