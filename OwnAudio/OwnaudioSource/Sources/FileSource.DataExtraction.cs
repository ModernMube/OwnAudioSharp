...
public partial class FileSource
{
    ...
    public byte[] GetByteAudioData(TimeSpan position, TimeSpan? duration = null)
    {
        ...
        try
        {
            ...
            int totalSamples = totalFrames * _streamInfo.Channels;
            int totalBytes = totalSamples * sizeof(float);

            byte[] resultData = new byte[totalBytes];
            int bytesWritten = 0;

            // Decode frames until we have enough data
            while (bytesWritten < totalBytes)
            {
                var result = tempDecoder.DecodeNextFrame();
                ...
                if (!result.IsSucceeded || result.Frame == null || result.Frame.Data == null)
                {
                    break;
                }

                // Copy frame data to result
                int bytesToCopy = Math.Min(result.Frame.Data.Length, totalBytes - bytesWritten);
                Buffer.BlockCopy(result.Frame.Data, 0, resultData, bytesWritten, bytesToCopy);
                bytesWritten += bytesToCopy;
            }

            // Return only the data we actually got
            if (bytesWritten < totalBytes)
            {
                Array.Resize(ref resultData, bytesWritten);
            }
