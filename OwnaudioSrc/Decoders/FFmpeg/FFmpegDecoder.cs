    ...
    public AudioDecoderResult DecodeNextFrame(int? frameNumber = 0)
    {
        lock (_syncLock)
        {
            ...
            for (int i = 0; i <= frameNumber; i++)
            {
                ...
                accumulatedData.Write(data, 0, data.Length);

                var pts = _currentFrame->best_effort_timestamp >= 0
                    ? _currentFrame->best_effort_timestamp
                    : (_currentFrame->pts >= 0 ? _currentFrame->pts : 0);

                lastPresentationTime = Math.Round(pts * ffmpeg.av_q2d(_formatCtx->streams[_streamIndex]->time_base) * 1000.0, 2);
            }

            var combinedData = accumulatedData.ToArray();
            _decoderAudioFrame = new AudioFrame(lastPresentationTime, combinedData);
