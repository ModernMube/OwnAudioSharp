...
namespace Ownaudio.Utilities.Matchering
{
    ...
    public partial class AudioAnalyzer
    {
        ...
        public AudioSpectrum AnalyzeAudioFile(string filePath)
        {
            ...
            var segments = CreateAudioSegments(monoData, sampleRate);
            var segmentAnalyses = AnalyzeSegments(segments, sampleRate);
            var filteredAnalyses = FilterOutlierSegments(segmentAnalyses);
        ...
        private List<AudioSegment> CreateAudioSegments(float[] audioData, int sampleRate)
        {
            ...
            for (int start = 0; start < audioData.Length - segmentSamples; start += hopSize)
            {
                ...
                var segmentData = new float[actualLength];
        ...
        {
            var analyses = new List<SegmentAnalysis>();
            int validSegments = 0;
            ...
            for (int i = 0; i < segments.Count; i++)
            {
                ...
                var spectrum = AnalyzeSegmentSpectrum(segment.Data, sampleRate);
                var dynamics = AnalyzeAbsoluteDynamics(segment.Data);
        ...
        private List<SegmentAnalysis> FilterOutlierSegments(List<SegmentAnalysis> analyses)
        {
            ...
            var filtered = new List<SegmentAnalysis>();
            int outlierCount = 0;

            // Calculate statistics for each frequency band
            for (int band = 0; band < FrequencyBands.Length; band++)
            {
                var bandValues = analyses.Select(a => a.FrequencySpectrum[band]).ToArray();
        ...
        private AudioSpectrum CalculateWeightedAverageSpectrum(List<SegmentAnalysis> analyses)
        {
            ...
            var weightedSpectrum = new float[FrequencyBands.Length];
        ...
        private float[] AnalyzeFrequencySpectrumAbsolute(float[] audioData, int sampleRate)
        {
            ...
            var windowFunction = GenerateFlatTopWindow(fftSize); // More accurate amplitude measurement
            ...
            for (int w = 0; w < windowCount; w++)
            {
                ...
                for (int band = 0; band < FrequencyBands.Length; band++)
                {
                    var energy = CalculateBandEnergyAbsolute(fftInput, FrequencyBands[band],
