...
namespace Ownaudio.Utilities
{
    public partial class WaveAvaloniaDisplay : Avalonia.Controls.Control
    {
            ...
            int channels = 1, int sampleRate = 44100)
        {
            ...
            if (preferFFmpeg)
            {
                return TryLoadWithFFmpeg(filePath, maxSamples, channels, sampleRate) ||
                       TryLoadWithMiniAudio(filePath, maxSamples, channels, sampleRate);
            }
            else
            {
                return TryLoadWithMiniAudio(filePath, maxSamples, channels, sampleRate) ||
                       TryLoadWithFFmpeg(filePath, maxSamples, channels, sampleRate);
            }
        }
        ...
        private bool TryLoadWithMiniAudio(string filePath, int maxSamples, int channels, int sampleRate)
        {
            MiniDecoder? decoder = null;
            ...
            try
            {
                ...
                decoder = new MiniDecoder(filePath, options);
        ...
        /// <summary>
        /// Tries to load audio file using FFmpeg decoder.
        /// </summary>
        /// <param name="filePath">Path to the audio file.</param>
        /// <param name="maxSamples">Maximum number of samples for visualization.</param>
        /// <param name="channels">Desired channel count.</param>
        /// <param name="sampleRate">Desired sample rate.</param>
        /// <returns>True if successful, false otherwise.</returns>
        private bool TryLoadWithFFmpeg(string filePath, int maxSamples, int channels, int sampleRate)
        {
            ...
            {
                var options = new FFmpegDecoderOptions(channels, sampleRate);
                decoder = new FFmpegDecoder(filePath, options);
            ...
            {
                System.Diagnostics.Debug.WriteLine($"FFmpeg decoder failed: {ex.Message}");
                return false;
            }
            finally
            {
        ...
        public bool LoadFromAudioStream(Stream stream, int maxSamples = 100000, bool preferFFmpeg = false,
            int channels = 1, int sampleRate = 44100)
        {
            if (stream == null)
            ...
            if (preferFFmpeg)
            {
                return TryLoadStreamWithFFmpeg(stream, maxSamples, channels, sampleRate) ||
                       TryLoadStreamWithMiniAudio(stream, maxSamples, channels, sampleRate);
            }
            else
            {
                return TryLoadStreamWithMiniAudio(stream, maxSamples, channels, sampleRate) ||
                       TryLoadStreamWithFFmpeg(stream, maxSamples, channels, sampleRate);
            }
        }
        ...
        /// <param name="sampleRate">Desired sample rate.</param>
        /// <returns>True if successful, false otherwise.</returns>
        private bool TryLoadStreamWithMiniAudio(Stream stream, int maxSamples, int channels, int sampleRate)
        {
            MiniDecoder? decoder = null;
            var tempBuffer = ArrayPool<float>.Shared.Rent(4096);
            var sampleList = new List<float>(maxSamples);
            ...
            {
                var options = new FFmpegDecoderOptions(channels, sampleRate);
                decoder = new MiniDecoder(stream, options);
        ...
        /// <summary>
        /// Tries to load audio stream using FFmpeg decoder.
        /// </summary>
        /// <param name="stream">Audio stream to load.</param>
        /// <param name="maxSamples">Maximum number of samples for visualization.</param>
        /// <param name="channels">Desired channel count.</param>
        /// <param name="sampleRate">Desired sample rate.</param>
        /// <returns>True if successful, false otherwise.</returns>
        private bool TryLoadStreamWithFFmpeg(Stream stream, int maxSamples, int channels, int sampleRate)
        {
            FFmpegDecoder? decoder = null;
            var tempBuffer = ArrayPool<float>.Shared.Rent(4096);
            var sampleList = new List<float>(maxSamples);
            ...
            {
                var options = new FFmpegDecoderOptions(channels, sampleRate);
                decoder = new FFmpegDecoder(stream, options);

                return ProcessDecoderData(decoder, tempBuffer, sampleList, maxSamples);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"FFmpeg stream decoder failed: {ex.Message}");
                return false;
            }
            finally
            {
