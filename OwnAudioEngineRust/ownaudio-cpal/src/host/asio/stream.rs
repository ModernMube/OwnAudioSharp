...
impl Device {
        ...
        E: FnMut(Error) + Send + 'static,
    {
        ...
        let callback_id = driver.add_callback(move |callback_info| unsafe {
                ...
                F: Fn(A) -> A,
            {
                ...
                let (stride, n_channels) = asio_channel_bounds(interleaved.len(), asio_stream);
                ...
                for ch_ix in 0..n_channels {
                    ...
                    for (frame, s_asio) in interleaved.chunks_mut(stride).zip(asio_channel) {
        ...
        let callback_id = driver.add_callback(move |callback_info| unsafe {
                ...
                F: Fn(A, A) -> A,
            {
                ...
                for ch_ix in 0..n_channels {
                    ...
                    for (frame, s_asio) in interleaved.chunks(stride).zip(asio_channel) {
    ...
    fn get_or_create_input_stream(
        ...
        match streams.input {
            Some(ref input) => {
                check_reuse(input, "input", num_channels, config.buffer_size)?;
                Ok(input.buffer_size as usize)
            }
            None => {
    ...
    fn get_or_create_output_stream(
        ...
        match streams.output {
            Some(ref output) => {
                check_reuse(output, "output", num_channels, config.buffer_size)?;
                Ok(output.buffer_size as usize)
            }
            None => {
...
/// Validate a new stream's request against an ASIO stream that is already open.
///
/// ASIO allocates a single buffer set per driver, so the first stream to open fixes
/// the channel count and buffer size for every later stream on that driver. cpal
/// used to hand the existing buffers back regardless of what was asked for, which
/// left the new stream's callback deriving a channel count and buffer size from its
/// own config and then indexing the *shared* buffer set with them — an out-of-bounds
/// write into driver-owned memory.
///
/// Rejecting here turns that silent memory-safety hazard into an `UnsupportedConfig`
/// at build time. This is a behaviour change: configurations that previously appeared
/// to work will now fail, which is the point — they were corrupting the heap.
fn check_reuse(
    existing: &sys::AsioStream,
    direction: &str,
    num_channels: usize,
    buffer_size: BufferSize,
) -> Result<(), Error> {
    if num_channels != existing.buffer_infos.len() {
        return Err(Error::with_message(
            ErrorKind::UnsupportedConfig,
            format!(
                "an ASIO {direction} stream with {existing_ch} channels is already open on this \
                 driver; {num_channels} channels were requested. ASIO allocates one buffer set \
                 per driver, so all streams must agree on the channel count.",
                existing_ch = existing.buffer_infos.len(),
            ),
        ));
    }
    if let BufferSize::Fixed(requested) = buffer_size {
        if requested as i32 != existing.buffer_size {
            return Err(Error::with_message(
                ErrorKind::UnsupportedConfig,
                format!(
                    "an ASIO {direction} stream with a buffer size of {existing_bs} is already \
                     open on this driver; {requested} was requested. All streams on one ASIO \
                     driver share a single buffer set and must agree on the buffer size.",
                    existing_bs = existing.buffer_size,
                ),
            ));
        }
    }
...
/// Number of buffers ASIO allocates per channel (double buffering).
const ASIO_NUM_BUFFERS: usize = 2;

/// The interleaving stride and the number of channels a callback may actually touch.
///
/// These are normally equal. They can only diverge if this stream's channel count
/// disagrees with the driver's buffer set — see the module docs on ASIO's
/// one-buffer-set-per-driver model, and `get_or_create_output_stream`, which rejects
/// such configurations up front.
///
/// The stride is deliberately *not* clamped: it describes the layout of cpal's own
/// interleaved buffer, and changing it would silently reorder channels. Only the loop
/// bound is clamped, so a mismatch degrades to "the channels the driver did not
/// allocate stay silent" instead of writing past the driver's buffers.
#[inline]
fn asio_channel_bounds(interleaved_len: usize, asio_stream: &sys::AsioStream) -> (usize, usize) {
    let buffer_size = asio_stream.buffer_size.max(0) as usize;
    if buffer_size == 0 {
        debug_assert!(false, "cpal/asio: buffer size of 0");
        return (1, 0);
    }
    let stride = interleaved_len / buffer_size;
    debug_assert_eq!(
        stride,
        asio_stream.buffer_infos.len(),
        "cpal/asio: stream channel count disagrees with the driver's buffer set",
    );
    // `chunks`/`chunks_mut` panic on a stride of 0, so never hand back 0.
    (stride.max(1), stride.min(asio_stream.buffer_infos.len()))
}

/// Resolve the pointer and element count for one channel of an ASIO buffer set,
/// bounded by what the driver actually allocated.
///
/// Returns `None` — meaning "build an empty slice" — when the request cannot be
/// satisfied safely. Every rejection is a bug on the cpal side, so each one trips a
/// `debug_assert` while iterating, but in release the callback must degrade to doing
/// nothing rather than panic: these run inside an `extern "C"` callback invoked by the
/// driver, and unwinding across that boundary is itself undefined behaviour.
///
/// The bound that matters is `channel_bytes`. `ASIOCreateBuffers` allocates exactly
/// `buffer_size * <driver sample width>` bytes per channel buffer; anything longer is
/// a write into driver-owned memory the driver does not own for us.
#[inline]
unsafe fn asio_channel_ptr<T>(
    asio_stream: &sys::AsioStream,
    buffer_index: usize,
    channel_index: usize,
    ...
    debug_assert!(
        channel_index < asio_stream.buffer_infos.len(),
        "cpal/asio: channel {channel_index} is outside the driver's buffer set of {}",
        asio_stream.buffer_infos.len(),
    );
    debug_assert!(
        buffer_index < ASIO_NUM_BUFFERS,
        "cpal/asio: driver supplied buffer index {buffer_index}",
    );

    let info = asio_stream.buffer_infos.get(channel_index)?;
    if buffer_index >= ASIO_NUM_BUFFERS {
        return None;
    }

    let channel_length = requested_channel_length.unwrap_or(asio_stream.buffer_size as usize);

    // The driver's guarantee, in elements of `T`. `channel_bytes` is 0 when the
    // driver reported a sample type of unknown width, which correctly yields 0 here.
    let size_of_t = std::mem::size_of::<T>();
    let available = if size_of_t == 0 {
        0
    } else {
        asio_stream.channel_bytes / size_of_t
    };
    debug_assert!(
        channel_length <= available,
        "cpal/asio: asked for {channel_length} elements of {} bytes from a channel buffer \
         the driver allocated {} bytes for",
        size_of_t,
        asio_stream.channel_bytes,
    );
    let channel_length = channel_length.min(available);

    let buff_ptr = info.buffers[buffer_index] as *mut T;
    debug_assert!(!buff_ptr.is_null(), "cpal/asio: null channel buffer");
    if buff_ptr.is_null() || channel_length == 0 {
        return None;
    }

    Some((buff_ptr, channel_length))
}
...
///
/// Returns an empty slice rather than an out-of-bounds one if the request does not
/// fit inside the driver's allocation; see [`asio_channel_ptr`].
///
/// # Safety
///
/// - `asio_stream` must describe buffers that are currently allocated, i.e. the
///   caller must hold the `asio_streams` lock for the duration of the borrow.
/// - `T` must be a valid bit pattern for the driver's sample type.
#[inline]
unsafe fn asio_channel_slice<T>(
    ...
    match asio_channel_ptr::<T>(
        asio_stream,
        buffer_index,
        channel_index,
        requested_channel_length,
    ) {
        Some((ptr, len)) => std::slice::from_raw_parts(ptr as *const T, len),
        None => &[],
    }
}
...
/// - `asio_stream` must describe buffers that are currently allocated, i.e. the
///   caller must hold the `asio_streams` lock for the duration of the borrow.
/// - No other slice over the same channel buffer may be alive at the same time.
/// - `T` must be a valid bit pattern for the driver's sample type.
#[inline]
unsafe fn asio_channel_slice_mut<T>(
    ...
    match asio_channel_ptr::<T>(
        ...
        Some((ptr, len)) => std::slice::from_raw_parts_mut(ptr, len),
        None => &mut [],
    }
}
    ...
    D: FnMut(&mut Data, &OutputCallbackInfo) + Send + 'static,
{
    ...
    for ch_ix in 0..n_channels {
        ...
        for (channel_sample, sample_in_buffer) in asio_channel
            .chunks_exact_mut(asio_sample_size_bytes)
            .zip(interleaved.iter().skip(ch_ix).step_by(stride))
        {
    ...
    D: FnMut(&Data, &InputCallbackInfo) + Send + 'static,
{
    ...
    // 1. Write the ASIO channels to the CPAL buffer.
    let interleaved: &mut [I24] = cast_slice_mut(interleaved);
    let n_frames = asio_stream.buffer_size.max(0) as usize;
    let (stride, n_channels) = asio_channel_bounds(interleaved.len(), asio_stream);
    let buffer_index = asio_info.buffer_index as usize;
    let asio_sample_size_bytes = 3;
    ...
    for ch_ix in 0..n_channels {
        ...
        for (channel_sample, sample_in_buffer) in asio_channel
            .chunks_exact(asio_sample_size_bytes)
            .zip(interleaved.iter_mut().skip(ch_ix).step_by(stride))
        {
            let sample = i24_bytes_to_i32(
                &[channel_sample[0], channel_sample[1], channel_sample[2]],
                little_endian,
...
mod tests {
    ...
    /// Backing storage standing in for the driver's allocation, so a violated
    /// bound would be a real out-of-bounds write rather than a dangling pointer.
    struct FakeDriverBuffers {
        _storage: Vec<Vec<u8>>,
        stream: sys::AsioStream,
    }

    /// Build an `AsioStream` describing `n_channels` channels of
    /// `buffer_size * sample_bytes` bytes each — exactly what `ASIOCreateBuffers`
    /// would hand back for a driver with that sample width.
    fn fake_stream(n_channels: usize, buffer_size: i32, sample_bytes: usize) -> FakeDriverBuffers {
        let channel_bytes = buffer_size as usize * sample_bytes;
        let mut storage: Vec<Vec<u8>> = (0..n_channels * ASIO_NUM_BUFFERS)
            .map(|_| vec![0u8; channel_bytes])
            .collect();
        let buffer_infos = (0..n_channels)
            .map(|ch| sys::AsioBufferInfo {
                is_input: 0,
                channel_num: ch as i32,
                buffers: [
                    storage[ch * 2].as_mut_ptr() as *mut _,
                    storage[ch * 2 + 1].as_mut_ptr() as *mut _,
                ],
            })
            .collect();
        FakeDriverBuffers {
    ...
    #[test]
    fn slice_matches_the_driver_allocation() {
        let f = fake_stream(2, 512, 4);
        for ch in 0..2 {
            for buf in 0..ASIO_NUM_BUFFERS {
                let slice = unsafe { asio_channel_slice::<i32>(&f.stream, buf, ch, None) };
                assert_eq!(slice.len(), 512, "one i32 per frame");
            }
        }
    }

    #[test]
    fn i24_slice_uses_the_requested_byte_length() {
        // A 3-byte driver format: the callback asks for `buffer_size * 3` bytes,
        // which is exactly the allocation.
        let f = fake_stream(2, 256, 3);
        let slice = unsafe { asio_channel_slice::<u8>(&f.stream, 0, 0, Some(256 * 3)) };
        assert_eq!(slice.len(), 256 * 3);
    }

    #[test]
    fn bounds_agree_when_the_stream_matches_the_driver() {
        let f = fake_stream(2, 512, 4);
        // 512 frames * 2 channels of interleaved samples.
        assert_eq!(asio_channel_bounds(1024, &f.stream), (2, 2));
    }

    // The remaining cases are configurations that must never reach the driver.
    // In a debug build they trip a `debug_assert`; in release they degrade to an
    // empty slice or a clamped loop bound. Both are checked, each in the build
    // where that behaviour is the defined one.

    #[test]
    #[cfg_attr(debug_assertions, should_panic(expected = "outside the driver's buffer set"))]
    fn channel_index_past_the_buffer_set_is_refused() {
        let f = fake_stream(2, 512, 4);
        let slice = unsafe { asio_channel_slice::<i32>(&f.stream, 0, 2, None) };
        assert!(slice.is_empty(), "release builds must not fabricate a slice");
    }

    #[test]
    #[cfg_attr(debug_assertions, should_panic(expected = "buffer index"))]
    fn buffer_index_past_the_double_buffer_is_refused() {
        let f = fake_stream(2, 512, 4);
        let slice = unsafe { asio_channel_slice::<i32>(&f.stream, 2, 0, None) };
        assert!(slice.is_empty(), "release builds must not fabricate a slice");
    }

    #[test]
    #[cfg_attr(debug_assertions, should_panic(expected = "the driver allocated"))]
    fn length_beyond_the_driver_allocation_is_clamped() {
        // A driver whose sample width is 2 bytes, read as if it were 4: the
        // allocation holds only half as many elements as the callback expects.
        // This is assumption A3 from ASIO_OUTPUT_BUFFER_OVERRUN.md, and the case
        // that turned into a heap overrun before `channel_bytes` existed.
        let f = fake_stream(2, 512, 2);
        let slice = unsafe { asio_channel_slice::<i32>(&f.stream, 0, 0, None) };
        assert_eq!(slice.len(), 256, "clamped to what the driver allocated");
    }

    #[test]
    #[cfg_attr(debug_assertions, should_panic(expected = "disagrees with the driver"))]
    fn more_channels_than_the_driver_allocated_clamps_the_loop_bound() {
        // Stream opened for 2 channels, callback interleaving 8 — section 4.3.
        let f = fake_stream(2, 512, 4);
        let (stride, n_channels) = asio_channel_bounds(512 * 8, &f.stream);
        assert_eq!(stride, 8, "the interleaved layout is unchanged");
        assert_eq!(n_channels, 2, "but only allocated channels are written");
    }

    #[test]
    #[cfg_attr(debug_assertions, should_panic(expected = "buffer size of 0"))]
    fn zero_buffer_size_yields_no_channels() {
        let f = fake_stream(2, 0, 4);
        let (stride, n_channels) = asio_channel_bounds(1024, &f.stream);
        assert_eq!(n_channels, 0);
        assert!(stride > 0, "chunks() panics on a stride of 0");
    }
}
