...
fn main() {
    // ASIO is Windows-only. Skip the real build on non-Windows platforms, on
    // docs.rs, and whenever the `sdk` feature is off.
    //
    // The `sdk` check is what keeps a plain `cargo build` of the enclosing
    // workspace from needing the Steinberg SDK: cargo builds this crate as an
    // auto-promoted workspace member even when no enabled dependency asks for it,
    // and the real path below downloads the SDK over the network. `cpal/asio`
    // enables `sdk`, so an actual ASIO build is unaffected.
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() != Ok("windows")
        || std::env::var("DOCS_RS").is_ok()
        || std::env::var("CARGO_FEATURE_SDK").is_err()
    {
