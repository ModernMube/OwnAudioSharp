...
pub struct AsioStream {
    ...
    /// Size of each buffer
    pub buffer_size: i32,
    /// The number of bytes the driver actually allocated for each half of each
    /// channel's double buffer, i.e. `buffer_size * <driver sample byte width>`.
    ///
    /// `ASIOCreateBuffers` guarantees exactly this much and no more. Callers that
    /// fabricate a slice over `buffer_infos[ch].buffers[i]` must not exceed it —
    /// the buffer is driver-owned memory and writing past it corrupts the heap of
    /// the host process. `0` means the driver reported a sample type whose width
    /// is not known, in which case no slice may be constructed at all.
...
impl AsioSampleType {
    /// The width in bytes of one sample of this type inside an ASIO channel buffer.
    ///
    /// `ASIOCreateBuffers` allocates `buffer_size * byte_width` bytes per channel per
    /// half of the double buffer, so this is what bounds any slice built over a
    /// driver buffer.
    ///
    /// Returns `None` for the 1-bit DSD packings, where a sample is not
    /// byte-addressable, and for the `LastEntry` sentinel.
    pub fn byte_width(&self) -> Option<usize> {
        use AsioSampleType::*;
        match self {
            ASIOSTInt16MSB | ASIOSTInt16LSB => Some(2),
            ASIOSTInt24MSB | ASIOSTInt24LSB => Some(3),
            ASIOSTInt32MSB
            | ASIOSTInt32LSB
            | ASIOSTFloat32MSB
            | ASIOSTFloat32LSB
            ...
            | ASIOSTInt32LSB18
            | ASIOSTInt32LSB20
            | ASIOSTInt32LSB24 => Some(4),
            ASIOSTFloat64MSB | ASIOSTFloat64LSB => Some(8),
            ASIOSTDSDInt8NER8 => Some(1),
            ASIOSTDSDInt8LSB1 | ASIOSTDSDInt8MSB1 | ASIOSTLastEntry => None,
        }
    }
}
...
impl Driver {
    ...
    fn create_streams(
        ...
        // How many bytes the driver guarantees per channel buffer. Queried after
        // `ASIOCreateBuffers` so the answer describes the buffers that were just
        // allocated, and recorded on the stream so the callback can bound its writes
        // instead of assuming `buffer_size * size_of::<T>()`.
        let channel_bytes = |driver: &Self, buffer_size: i32, is_input: bool| -> usize {
            let width = if is_input {
                driver.input_data_type()
            } else {
                driver.output_data_type()
            };
            match width.ok().and_then(|t| t.byte_width()) {
                Some(w) => buffer_size.max(0) as usize * w,
                // Unknown sample width: report zero so every slice built over these
                // buffers comes out empty rather than guessed. cpal only builds
                // streams for sample types with a known width, so this cannot
                // silence an otherwise working stream.
                None => 0,
            }
        };
        ...
        let (input, output) = match (
            ...
            (false, false) => {
                ...
                let input = Some(AsioStream {
                    ...
                    buffer_size,
                    channel_bytes: channel_bytes(self, buffer_size, true),
                });
                let output = Some(AsioStream {
                    buffer_infos: output_buffer_infos,
                    buffer_size,
                    channel_bytes: channel_bytes(self, buffer_size, false),
                });
                (input, output)
            }
            // Just input
...
/// Checks the hand-written `repr(C)` mirrors against the SDK's generated types.
///
/// Only meaningful with the `sdk` feature: without it `ai::*` are the stub
/// bindings, whose layouts are placeholders and prove nothing.
#[test]
#[cfg(feature = "sdk")]
fn check_type_sizes() {
    assert_eq!(
